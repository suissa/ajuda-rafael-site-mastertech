...
$(document).ready(function(){
  ...
  $( `#menu` ).on( `click`, () => {
    $( `#menu` ).hide()
    $( `.barraLateral` ).css(`margin-left`, `1vw`)
  })

  $( `.conteudo-link` ).on( `click`, () => {
    $( `#menu` ).show()
    $( `.barraLateral` ).css(`margin-left`, `-25vw`)
  })
  // $('pre.code').highlight({source:1, zebra:1, indent:'space', list:'ol'});
  let hash = location.hash

  if ( hash !== "" ) {
    console.log('tem HASH', hash);
    // console.log(`lod: `, hash.replace( `#`, `` ).split(`_`))
    let content = hash.replace( `#`, `` ).split(`_`)

    $(`#menu-${content[0]}-container`).find('ul').slideToggle();
    $(`#menu-${content[0]}-container`).toggleClass('open');

    content = content[0] + `/` + hash.replace( `#`, `` )
    console.log(`content: `, content)
    const PATH = 'conteudos/'+content+'.html';
